...
describe 'api/v1/shapes' do
  ...
  describe :create do
    context 'when is successfully created' do
      before do
        post api_v1_diagram_shapes_path(diagram_id: diagram,
                                        shape: attributes_for(:shape))
      end
    ...
    context 'when is not created at all' do
      before do
        post api_v1_diagram_shapes_path(
          diagram_id: diagram,
          shape: attributes_for(:shape, kind: :square)
        )
      end
  ...
  describe :update do
    ...
    context 'when is successfully updated' do
      ...
      before do
        put api_v1_diagram_shape_path(diagram_id: shape_to_update.diagram,
                                      id: shape_to_update,
                                      shape: { label: new_value })
      end
